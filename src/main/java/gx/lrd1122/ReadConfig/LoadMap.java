    ...
    public static void LoadMapConfig(boolean sendLog) {
        ...
        if(MapFiles.length>0) {
            ...
            for (int i = 0; i < MapFiles.length; i++) {
                ...
                game.setCommandinstart(new ArrayList<>());
                game.setCommandinstart(new ArrayList<>());
                game.setWinners(new ArrayList<>());
                if(fileConfiguration.get("DisplayName") == null)
                {
                    game.setDisplayname(game.getName());
                }
                if(fileConfiguration.get("DisplayName") != null)
                {
                    game.setDisplayname(fileConfiguration.getString("DisplayName").replace("&", "§"));
                }
                if(fileConfiguration.get("StartCommands") != null) {
                    game.setCommandinstart(fileConfiguration.getStringList("StartCommands"));
                }
                if(fileConfiguration.get("EndComamnds") != null) {
                    game.setCommandinend(fileConfiguration.getStringList("EndCommands"));
                }
                game.setMaxplayers(fileConfiguration.getStringList("StartLocation").size());
                ...
                if(fileConfiguration.get("CommandInStart") != null) {
                    for(int e=0; e <fileConfiguration.getStringList("CommandInStart").size(); e++) {
                        String[] loc = fileConfiguration.getStringList("CommandInStart").get(e).split("");
                        Location location = new Location(Bukkit.getWorld(loc[0]), Double.valueOf(loc[1]), Double.valueOf(loc[2]), Double.valueOf(loc[3]));
                        game.setEndlocation(location);
                    }
                }
